...
export const typeDefs = gql`
  ...
  @sqlmancer {
    ...
    customScalars: { Date: ["DateTime"], JSON: ["JSON"] },
  }
  """
  type Query {
    """
  ...
  """
  @model {
    cte: "SELECT concat('customer_', customer_id) as customer_id, first_name, last_name, email, last_update, 'Customer' as __typename FROM customer UNION SELECT concat('actor_', actor_id) as customer_id, first_name, last_name, NULL as email, last_update, 'Actor' as __typename FROM actor",
    pk: "customer_id",
    include: ["__typename"],
  }
  """
...
export const schema = makeSqlmancerSchema({ typeDefs, resolvers })
